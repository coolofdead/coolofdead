...
function _SendEventClaim() {
 dataLayer.push({
  "event": "5-actions-claim_reward"
 });
}

function _SendEventEnd(str) {
 dataLayer.push({
  "event": UTF8ToString(str)
 });
}

function _SendEventJoin() {
 dataLayer.push({
  "event": "7-join_idoles"
 });
}

function _SendEventRestart() {
 dataLayer.push({
  "event": "5-actions-keep_playing"
 });
}

function _SendEventReward() {
 dataLayer.push({
  "event": "6-claim_reward"
 });
}

function _SendEventSkin(str) {
 dataLayer.push({
  "event": UTF8ToString(str)
 });
}

function _SendEventStart() {
 dataLayer.push({
  "event": "1-start-play"
 });
}

function _SendEventStyle(str) {
 dataLayer.push({
  "event": UTF8ToString(str)
 });
}
...
function _SetupFullscreen() {
 if (/Android/i.test(navigator.userAgent)) {
  var containerUnity = document.querySelector("#unity-container");
  containerUnity.requestFullscreen();
  ...
  var canvasUnity = document.querySelector("#unity-canvas");
  canvasUnity.style.width = window.innerWidth + "px";
  canvasUnity.style.height = window.innerHeight + "px";
 }
}
...
var asmLibraryArg = {
 "IsLandscapeOrientation": _IsLandscapeOrientation,
 "IsMobile": _IsMobile,
 ...
 "SendEventClaim": _SendEventClaim,
 "SendEventEnd": _SendEventEnd,
 "SendEventJoin": _SendEventJoin,
 "SendEventRestart": _SendEventRestart,
 "SendEventReward": _SendEventReward,
 "SendEventSkin": _SendEventSkin,
 "SendEventStart": _SendEventStart,
 "SendEventStyle": _SendEventStyle,
 "SetupFullscreen": _SetupFullscreen,
